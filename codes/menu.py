...
class BaseMenu:
    ...
    def show_path(self):
        path = ' > '.join([type(menu).__name__ for menu in self.history]) or 'Main Menu'
        print(path)
    ...
    def execute_choice(self, choice):
        if choice in self.options:
            ...
            if len(BaseMenu.history) > 0:
                if BaseMenu.history[-1] != self:
...
class ManageLoansMenu(BaseMenu):
    ...
    def __init__(self, db):
        super().__init__(db)
        self.options = {
            '1': ('pending', self.get_loans_by_status),
            '2': ('approved', self.get_loans_by_status),
            '3': ('declined', self.get_loans_by_status),
            '4': ('returned', self.get_loans_by_status),
            '5': ('edit status', self.edit_loan_status),
            '6': ('back', self.back)
        }
